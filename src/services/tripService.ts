...
export class TripService {
  ...
  async searchTrips(origin: string, destination: string, sort_by: string) {
    ...
    */
    if (!origin || !SUPPORTED_PLACES.has(origin.toUpperCase())) {
      throw new HttpError(400, `Airport does not exist: origin '${origin}'`);
    }
    if (!destination || !SUPPORTED_PLACES.has(destination.toUpperCase())) {
      throw new HttpError(400, `Airport does not exist: destination '${destination}'`);
    }
    ...
    */
    ...
    try {
      ...
      const sorted = results.sort((a: any, b: any) => {
        if (sort_by === 'fastest') return a.duration - b.duration;
        if (sort_by === 'cheapest') return a.cost - b.cost;
        return 0;
      });
