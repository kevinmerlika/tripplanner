...
class TripService {
    ...
    async searchTrips(origin, destination, sort_by) {
        /* So here i added validations so it should support only places included on the api, i stored everything into a set but it can be done by using |
       ...
       */
        ...
        //firstly trying to find the there is any key on redis based on the request parameters, if found then just return that as response so we just minimize 3rd API Calls
        ...
        //creating the url format for get request so all the parameters are sent to the provided 3rd party API 
        ...
        try {
            ...
            //Sorting results based on what user sent as parameter, it is done by using arrays Sort function. Also the same for cheapest we will get the lowest ones.
            ...
            //storing key into redis so next time we try to fetch this api with the same request body it will find the response on RAM, basically where redis cache is stored
